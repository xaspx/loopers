...
4. **Local Policy Engine (OPA/Rego) & Declarative YAML Guardrails**: Write fine-grained, attribute-based access control (ABAC) policies via custom Rego files or simplify agent governance using **Declarative YAML Policy Cards** (`policies.yaml`). Supports hot-reload, regular expressions, and **stateful session sequence constraints** (FSM sequence gating) out-of-the-box. See the **[Policy Engine Guide](./Documentation/docs/guides/policy-engine.md)**.
